...
/*   Updated: 2022/05/26 20:52:36 by wmardin          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
Variables:
	-	Static char remainder has 1024 spaces for the max amount of possible
		fd. Since it is static, it will keep its contents for the duration
		of the program, allowing for multiple fd to be read in parallel.
	-	Readbuffer is statically allocated, because it will always have
		the same size.
...
Structure
	-	Line must be actively nulled because it can get passed as an
		argument in its own initialization (while readreturn loop).
	-	Check if there is a remainder from prior calls to this fd
		-	If so, the remainder is copied into the workhorse "line"
			and freed and nulled.
		-	There might be a \n in that remainder so we might have
			to output something.
		-	If there is no \n, we move on to the read part.
	-	First call to read, during which we store the bytes read in
		readreturn.
	-	The while loop has 2 exit conditions:
		-	nothing left to read (readreturn no longer > 0)
